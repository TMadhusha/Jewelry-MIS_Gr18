    ...
    const filterAndSearchOrders = () => {
        ...
        if (search) {
            result = result.filter(order =>
                ...
                (order.remoteCustomers && `${order.remoteCustomers.firstname} ${order.remoteCustomers.lastname}`.toLowerCase().includes(search.toLowerCase()))
            );
        }
                        ...
                        <tbody>
                            {filteredOrders.map(order => (
                                <tr key={order.orderId} className={getOrderRowClassName(order.orderStatus)} onClick={() => handleRowClick(order)}>
                                    <td>{order.orderId}</td>
                                    <td>{order.remoteCustomers ? `${order.remoteCustomers.firstname} ${order.remoteCustomers.lastname}` : 'N/A'}</td>
